...
const Navbar = () => {
  ...
  return (
    <>
      <header className={
        !navState ? 'absolute top-7 left-0 right-0 opacity-100 z-50' : 'fixed top-0 left-0 right-0 h-[9vh] flex items-center justify-center opacity-100 z-[500] blur-effect-theme'
      }>
            ...
            <ul className="flex items-center justify-center gap-2">
              ...
              <li className="grid items-center">
                <button type='button' className="border-none outline-none active:scale-110 transition-all duration-300 relative">
                  ...
                  <span className={`absolute top-4 right-0  w-4 h-4 text-[0.65rem] shadow leading-tight font-medium rounded-full flex items-center justify-center cursor-pointer hover:scale-110 transition-all duration-300 ${navState ? 'bg-slate-900 text-slate-100 shadow-slate-900' : 'bg-slate-100 text-slate-900  shadow-slate-100'}`}>0</span>
                </button>
              </li>
            </ul>
        </nav>
